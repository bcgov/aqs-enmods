...
core_data AS (
    SELECT DISTINCT
        ...
        CASE 
            WHEN m.enmods_medium = 'Plant Tissue - Terrestrial' THEN 'Plant - Terrestrial'
...
--		END AS "Result Unit",
        ...
        result.tax_nm_cd,
        result.tax_nm_cd_tokens,   -- ADD THIS
        smpl.lab_arrival_temperature                                 AS "Lab Arrival Temperature",
        result.result_comment AS "Lab Quality Flag",-- leave blank
        CASE 
...
--        "Field Preservative",
        ...
        '' as "Activity ID",
        CASE 
            WHEN UPPER("Medium") LIKE '%WATER%' THEN to_char("Activity Name")||'P'
...
--    END AS "Lab Sample ID",
       ...
       "DEBUGGING TAX_NM_CD",
        ...
        ROW_NUMBER() OVER (PARTITION BY "Work Order Number", "Location ID", "Activity Name", "Observed DateTime", "Observed Property ID", "Result Value" ORDER BY "Lab Quality Flag") AS rn_dup,     
                     COUNT(*) OVER (PARTITION BY "Observed Property ID") AS cnt_op,
        ROW_NUMBER() OVER (PARTITION BY "Observed Property ID" ORDER BY "DEBUGGING TAX_NM_CD") AS rn_op--_tax_nm_cd--,
--      rn_taxa_dup
from(select DISTINCT
    "Observation ID",
        ...
        "Collection Method",
        CASE 
            WHEN "Observed Property ID" IN ('Extinction Depth (len.)', 'Temperature (temp.)') THEN 'Water - Unknown'
...
--      CASE 
--          WHEN "Observed Property ID 2" <> 'Taxonomy' THEN 0
--          ELSE rn_taxa_dup
--      END AS rn_taxa_dup        
from(select DISTINCT
    "Observation ID",
...
select distinct "Observation ID",
       ...
       "Method Reporting Limit",
        CASE 
          WHEN "Observed Property ID 2" = 'Taxonomy' AND "PARM_CD" = '-111' AND "Medium" = 'Plant - Phytoplankton' THEN 'cells/mL'
          WHEN "Observed Property ID 2" = 'Taxonomy' AND "PARM_CD" = '-111' AND "Medium" = 'Plant - Periphyton' THEN 'cells/cm2'
...
FROM -- fish data
    (
        (
            ...
            WHERE upper(core."Medium") LIKE '%PLANT%'
            ) core_updated
...
--                AND tax_cw.parm_code is NOT NULL
--                 AND INSTR(
--       '|' || REGEXP_REPLACE(UPPER(tax_cw.parm_code), '[^A-Z0-9]+', '|') || '|',
--       '|' || UPPER(core_updated.tax_nm_cd) || '|'
--     ) > 0
                --AND UPPER(tax_cw.parm_code) LIKE '%' || upper(core_updated.tax_nm_cd) || '%'
--left outer join ems.ems_etl_tax_crosswalk_temp tax_cw on core_updated.tax_nm_cd = tax_cw.parm_code
                )
WHERE upper(core_updated."Medium") LIKE '%PLANT%'
...
AND upper(core_updated."Medium") LIKE '%PLANT%'
...
-- end fish data
 AND ed.NewNameID is not null
    ...
    --AND "Work Order Number" LIKE '%50059977%' --10068651
    )
...
-- BIO_SAMPLE_WEIGHT, Composite Stat, CONTINUOUS_MAXIMUM, Air Flow Volume, Air Filter Size, LIFE_STG_CD, WEIGHT_FROM, and SIZE_FROM are all NULLS for Plant planktons
