...
get_profiles_for_url <- function(env, url){
  ...
  x_temp <- httr::GET(url, config = c(httr::add_headers(.headers = c('Authorization' = token))),
  ...
  if (total > 1000) { #if there are more than 1000 records loop
    ...
    for (i in seq(2,number_loops)) {
      ...
      x_temp <- httr::GET(temp_url, config = c(httr::add_headers(.headers =
...
del_profiles <- function(env, operation, data_type){
  ...
  for(id in del_ids){
    ...
    #Make the unit group
    x<-httr::DELETE(url_id, config = c(httr::add_headers(.headers = c('Authorization' = token))),
              body = data_body, encode = 'json')
...
put_profiles <- function(env, data_type, profile){
  ...
  if(data_type %in% c("analysis_methods", "observed_properties",
  ...
  } else if((data_type %in% c("result_grades", "result_statuses",
                               "mediums"))) {
  ...
  #return()
    ...
    profile <- profile %>% left_join(analysis_methods_get %>%
    ...
    profile <- profile %>% left_join(observed_properties_get %>%
    ...
    profile <- profile %>%
      { if ("unit_group_id" %in% colnames(.))
        dplyr::select(., -unit_group_id) else . } %>%
    ...
    profile <- profile %>%
      { if ("unit_id" %in% colnames(.))
        dplyr::select(., -unit_id) else . } %>%
      dplyr::left_join(., units,
                       by = join_by('sample_unit' == 'customId'),
                       keep = FALSE)

    rel_var <- c("id", "parm_code", "newnameid", "description", "analysis_type",
    ...
    profile <- profile %>%
      dplyr::left_join(observed_properties %>%
                         dplyr::select(id, customId) %>%
    ...
    profile <- profile %>% left_join(analytical_groups_get %>%
    ...
    profile <- profile %>% dplyr::select(-id) %>%
    ...
    profile <- profile %>% left_join(labs_get %>%
                  dplyr::select(id, customId),
                  by = join_by("short_name" == "customId"))

    rel_var <- c("id", "short_name", "name", "description", "address", "contact_name",
                 "email", "phone_number")
    ...
    for(j in 1:dim(profile)[1]){
      ...
      data_body <- list("customId" = temp_profile$short_name,
                        "name" = temp_profile$name,
                        "description" = temp_profile$description,
...
post_profiles <- function(env, data_type, profile){
  ...
  if(data_type == "unit_groups"){
    ...
    profile <- profile %>%
      dplyr::left_join(unitgroups_profiles %>%
                  dplyr::select(id, customId) %>%
    ...
    rel_var <- c("short_name", "name", "description", "address", "contact_name",
                 "email", "phone_number")
    ...
    profile <- profile %>%
      dplyr::left_join(taxonomylevels_profiles %>%
                  dplyr::select(id, customId) %>%
    ...
    rel_var <- c("customId", "name", "type", "startTime", "endTime",
  ...
  for(j in 1:dim(profile)[1]){
    ...
    #Post the configuration
    x<-httr::POST(url, config = c(httr::add_headers(.headers =
                                          c('Authorization' = token))), body = data_body, encode = 'json')
