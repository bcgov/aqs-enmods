...
options(java.parameters = "-Xmx4g") # 4 GB heap
...
library(plotly)
library(tidyr)
# ---- Help Desk (Outlook PST) analysis dependencies ----
library(purrr)
library(stringdist)
library(rJava)
library(freepst)
...
# ---- 1b) Help Desk (Outlook PST) analysis pipeline ----
# Produces `linked` (matched request->response pairs) and `inbox_missing` (incoming with no contact-matched
# response), which drive Section 2.1 of the report.

# freepst::read_folder expects a path to a .pst. Shiny stores uploads under a
# temp path with no .pst extension, so copy to a .pst-suffixed temp file first.
hd_stage_pst <- function(upload_path) {
  dest <- tempfile(fileext = ".pst")
  ok <- file.copy(upload_path, dest, overwrite = TRUE)
  if (!ok) stop("Could not stage PST upload to a .pst temp file.")
  dest
}

# Clean the freepst delivery_time strings into America/Vancouver POSIXct.
hd_clean_times <- function(x) {
  no_tz <- stringr::str_replace(x, " [A-Z]{3,4} ", " ")
  no_tz <- lubridate::parse_date_time(no_tz, orders = "a b d H:M:S Y", tz = "UTC")
  lubridate::force_tz(no_tz, "America/Vancouver")
}

# Read + normalise a mailbox folder. mail_box is "Incoming Mail"/"Outgoing Mail".
hd_read_mailbox <- function(pst_path, mail_box, floor_date) {
  items <- freepst::read_folder(pst_path) %>%
    dplyr::rename(
      folder_name   = folder,
      sender_name   = sent_by,
      sender_email  = sent_by_addr,
      received_time = delivery_time
    ) %>%
    dplyr::mutate(mail_box = mail_box)

  items$received_time <- hd_clean_times(items$received_time)
  items$sender_name <- str_to_lower(items$sender_name)
  items %>% dplyr::filter(received_time >= floor_date)
}

# Build the users lookup (name_email_parts drives contact resolution).
hd_prepare_users <- function(users_path) {
  read.csv(users_path, stringsAsFactors = FALSE) %>%
    dplyr::mutate(Name = iconv(Name, from = "", to = "UTF-8", sub = "")) %>%
    dplyr::mutate(
      full_name = tolower(Name),
      email     = tolower(Email)
    ) %>%
    dplyr::mutate(name_email = stringr::str_c(full_name, " ", email)) %>%
    dplyr::mutate(name_email_parts = stringr::str_split(name_email, "\\s+")) %>%
    dplyr::mutate(name_email_parts = stringr::str_squish(name_email_parts)) %>%
    dplyr::mutate(
      name_email_parts = stringr::str_remove_all(name_email_parts, 'c\\(|\\)|"') %>%
        stringr::str_split(",\\s*")
    )
}

# Emails to drop from any resolved-contact set (help desk staff / system).
# Read from the user-supplied exclusion CSV (uploaded via the dashboard).
hd_read_excluded_emails <- function(excluded_csv) {
  read_csv(excluded_csv) %>%
    dplyr::select(Email) %>%
    unlist() %>%
    str_to_lower()
}

# Resolve contacts on SENT items via the PidTagDisplayTo recipient list.
hd_resolve_sent_contacts <- function(sent_items, users, hd_excluded_emails) {
  sent_items <- sent_items %>%
    dplyr::mutate(
      display_to = stringr::str_extract(tostr, "PidTagDisplayTo:001F:([^\\r\\n]+)") %>%
        stringr::str_remove("PidTagDisplayTo:001F:") %>%
        stringr::str_replace_all("[,;]", " ") %>%
        stringr::str_remove_all("\\s+\\S+:EX") %>%
        stringr::str_squish()
    ) %>%
    dplyr::mutate(
      display_to = purrr::map(display_to, function(x) {
  ...
  sent_items %>%
    dplyr::mutate(
      extracted_contacts = purrr::map_chr(display_to, function(x) {
        if (is.null(x) || length(x) == 0 || all(is.na(x))) {
          return(NA_character_)
        }
        x <- tolower(x) %>% stringr::str_squish()

        extracted_contacts <- users %>%
          dplyr::filter(
            purrr::map_lgl(name_email_parts, function(parts) {
              if (is.null(parts) || length(parts) == 0) {
                return(FALSE)
              }
              parts <- tolower(parts)
              n_matches <- sum(x %in% parts)
              if (length(x) == 1) {
                n_matches >= 1
              } else {
                n_matches >= 2
        ...
        extracted_contacts <- unique(extracted_contacts)
        extracted_contacts <- extracted_contacts[!(extracted_contacts %in% hd_excluded_emails)]
        paste(unique(extracted_contacts), collapse = "; ")
      })
    )
}

# Resolve contacts on INBOX items via the message body (tostr) name/email match.
hd_resolve_inbox_contacts <- function(inbox_items, users, hd_excluded_emails, excluded_csv) {
  sender_names_remove <- read_csv(excluded_csv) %>%
    dplyr::select(Name) %>%
    unlist() %>%
    str_to_lower()

  inbox_items <- inbox_items %>%
    dplyr::filter(!(sender_name %in% sender_names_remove))

  inbox_items <- inbox_items %>%
    dplyr::mutate(
      extracted_contacts = purrr::map_chr(tostr, function(x) {
        if (is.na(x)) {
          return(NA_character_)
        }
        text <- tolower(x)

        matched_emails <- users %>%
          dplyr::filter(
            purrr::map_lgl(name_email_parts, function(parts) {
              if (is.null(parts) || length(parts) == 0) {
                return(FALSE)
              ...
              parts <- tolower(parts)
              matches <- stringr::str_detect(text, fixed(parts))
              ...
              cond2 <- FALSE
              if (n_matches == 1) {
                matched_part <- parts[matches]
                cond2 <- length(matched_part) == 1 &&
                  !is.na(matched_part) &&
                  stringr::str_detect(matched_part, "@")
              }
              cond1 || cond2
            })
          ) %>%
        ...
        matched_emails <- unique(matched_emails)
        matched_emails <- matched_emails[!(matched_emails %in% hd_excluded_emails)]
        paste(matched_emails, collapse = "; ")
      })
    )

  # Append the raw sender email as a fallback contact token.
  inbox_items %>%
    dplyr::mutate(extracted_contacts = stringr::str_c(extracted_contacts, ";", sender_email))
}

# Subject-cleaning helper used in subject-similarity scoring.
hd_clean_subject <- function(x) {
  x %>%
    iconv(from = "", to = "UTF-8", sub = "") %>%
    stringr::str_to_lower() %>%
    stringr::str_remove_all(stringr::regex("(re:|fw:|fwd:)\\s*|\\[external\\]", ignore_case = TRUE)) %>%
    stringr::str_squish()
}

# The verbatim person-specific sender_name -> email mapping. Retained because it
# raises the match rate in `linked` and shrinks `inbox_missing` (which drives the
# oldest-pending timestamp).
hd_apply_sender_overrides <- function(inbox_items, overrides_path) {
  if (!file.exists(overrides_path)) {
    stop(
      "Sender overrides (Name to email translator) CSV not found at: ",
      overrides_path
    )
  }
  ...
  overrides <- readr::read_csv(
    ...
    col_types = readr::cols(
      sender_name = readr::col_character(),
      email       = readr::col_character()
    )
  )

  inbox_items %>%
    dplyr::left_join(overrides, by = "sender_name") %>%
    dplyr::mutate(
      extracted_contacts = dplyr::coalesce(email, extracted_contacts),
      extracted_contacts = stringr::str_to_lower(extracted_contacts)
    ) %>%
    dplyr::select(-email)
}

# Subjects to exclude from sent items (bulletins / broadcasts, not replies).
hd_filter_subjects <- function(items) {
  items %>%
    dplyr::mutate(extracted_contacts = stringr::str_to_lower(extracted_contacts)) %>%
    dplyr::filter(!stringr::str_detect(subject, "New EDT User Requested")) %>%
    dplyr::filter(!stringr::str_detect(subject, "EnMoDS EDT back online but has a backlog")) %>%
    dplyr::filter(!stringr::str_detect(subject, "EnMoDS update")) %>%
    dplyr::filter(!stringr::str_detect(subject, "EnMoDS EDT Technical Bulletin"))
}

# Full pipeline: from staged PST paths + users CSV path to (linked, inbox_missing).
hd_run_pipeline <- function(inbox_pst, sent_pst, users_csv, excluded_csv, overrides_csv) {
  floor_date <- as.POSIXct("2026-01-01")

  users <- hd_prepare_users(users_csv)

  # Emails to drop from any resolved-contact set (from uploaded exclusion CSV).
  hd_excluded_emails <- hd_read_excluded_emails(excluded_csv)

  # --- Sent side ---
  sent_items <- hd_read_mailbox(sent_pst, "Outgoing Mail", floor_date)
  sent_items <- hd_resolve_sent_contacts(sent_items, users, hd_excluded_emails)
  sent_items <- sent_items %>%
    dplyr::filter(!(extracted_contacts == "")) %>%
    dplyr::filter(!is.na(extracted_contacts))

  # --- Inbox side ---
  inbox_items <- hd_read_mailbox(inbox_pst, "Incoming Mail", floor_date)
  inbox_items <- hd_resolve_inbox_contacts(inbox_items, users, hd_excluded_emails, excluded_csv)
  inbox_items <- inbox_items %>%
    dplyr::filter(!is.na(extracted_contacts)) %>%
    dplyr::filter(stringr::str_trim(extracted_contacts) != "")

  # Cap pathological recipient lists (<= 20 contacts) before exploding.
  sent_items <- sent_items %>%
    dplyr::filter(purrr::map_int(stringr::str_split(extracted_contacts, ";\\s*"), length) <= 20)
  inbox_items <- inbox_items %>%
    dplyr::filter(purrr::map_int(stringr::str_split(extracted_contacts, ";\\s*"), length) <= 20)

  # Explode one row per contact.
  sent_items <- sent_items %>%
    tidyr::separate_rows(extracted_contacts, sep = ";\\s*") %>%
    unique()
  ...
  # Person-specific sender overrides (verbatim, raises match rate).
  inbox_items <- hd_apply_sender_overrides(inbox_items, overrides_csv)

  inbox_items <- hd_filter_subjects(inbox_items)

  sent_items <- hd_filter_subjects(sent_items)

  # --- Link incoming -> outgoing on shared contact ---
  linked <- inbox_items %>%
    dplyr::inner_join(sent_items,
      by = "extracted_contacts",
      suffix = c("_inbox", "_sent")
    ) %>%
    unique()

  # inbox_missing: incoming with no contact-matched sent item (drives oldest_pending).
  inbox_missing <- inbox_items %>%
    dplyr::anti_join(linked,
      by = c(
        "sender_name"   = "sender_name_inbox",
        "received_time" = "received_time_inbox"
      )
    )
  # ) %>%
  # dplyr::filter(!str_detect(subject, "New EDT User Requested")) %>%
  # dplyr::filter(!str_detect(subject, "EnMoDS EDT back online but has a backlog")) %>%
  # dplyr::filter(!str_detect(subject, "EnMoDS update")) %>%
  # dplyr::filter(!str_detect(subject, "EnMoDS EDT Technical Bulletin"))

  # Keep only pairs where the response came after the request.
  linked <- linked %>%
    dplyr::filter(received_time_sent > received_time_inbox) %>%
    dplyr::mutate(time_diff = received_time_sent - received_time_inbox)

  list(linked = linked, inbox_missing = inbox_missing)
}

# Compute the Section 2.1 metrics from the pipeline output.
#   requests_addressed: distinct in-period matched requests in `linked`,
#                       gated on the inbox (request-arrival) timestamp.
# oldest_pending: up to five earliest received_time values in `inbox_missing`, overall (no period).
hd_compute_metrics <- function(linked, inbox_missing, start_van, end_van) {
  requests_addressed <- linked %>%
    dplyr::filter(
      received_time_inbox >= start_van,
      received_time_inbox <= end_van
    ) %>%
    dplyr::distinct(received_time_inbox) %>%
    nrow()

  # --- Statistical helpdesk response time (in days) ---
  # Reproduces the standalone outlook_analyzer pair selection: gate on subject
  # similarity (Jaro-Winkler >= 0.8), keep the LAST response per request, then
  # compute response time over pairs whose request (inbox) timestamp falls in
  # the user-selected window.
  response_time <- list(
    median = NA_real_, mean = NA_real_, p10 = NA_real_, p90 = NA_real_, n = 0L
  )

  if (!is.null(linked) && nrow(linked) > 0) {
    final_links <- linked %>%
      dplyr::mutate(
        subject_inbox_clean = hd_clean_subject(subject_inbox),
        subject_sent_clean = hd_clean_subject(subject_sent),
        subject_similarity = 1 - stringdist::stringdist(
          subject_inbox_clean, subject_sent_clean,
          method = "jw"
        )
      ) %>%
      dplyr::filter(!(subject_similarity < 0.8)) %>%
      # last response
      dplyr::group_by(extracted_contacts, received_time_inbox) %>%
      dplyr::slice_max(time_diff, n = 1, with_ties = TRUE) %>%
      dplyr::ungroup() %>%
      dplyr::group_by(extracted_contacts, received_time_sent) %>%
      dplyr::slice_max(received_time_inbox, n = 1, with_ties = TRUE) %>%
      dplyr::ungroup() %>%
      # same window as "Requests addressed", gated on request-arrival time
      dplyr::filter(
        received_time_inbox >= start_van,
        received_time_inbox <= end_van
      ) %>%
      dplyr::mutate(
        response_time_days = as.numeric(
          difftime(received_time_sent, received_time_inbox, units = "days")
        )
      )

    rt <- final_links$response_time_days
    rt <- rt[!is.na(rt)]
    if (length(rt) > 0) {
      response_time <- list(
        median = stats::median(rt),
        mean   = mean(rt),
        p10    = stats::quantile(rt, 0.10, names = FALSE),
        p90    = stats::quantile(rt, 0.90, names = FALSE),
        max    = stats::quantile(rt, 1, names = FALSE),
        n      = length(rt)
      )
    }
  }

  oldest_pending <- if (!is.null(inbox_missing) && nrow(inbox_missing) > 0) {
    # min(inbox_missing$received_time, na.rm = TRUE)
    head(sort(inbox_missing$received_time %>%
      format("%Y-%m-%d", tz = "America/Vancouver") %>%
      unique()), 5)
  } else {
    NA
  }
  ...
  list(
    requests_addressed = requests_addressed,
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    # ---- Help Desk mailbox uploads (for Section 2.1 of the report) ----
    fileInput(
      "file_inbox_pst",
      tags$span(
        tags$b("Upload Help Desk Inbox PST"), tags$br(),
        tags$small("Incoming mail (.pst)")
      ),
      multiple = FALSE,
      accept = c(".pst")
    ),
    fileInput(
      "file_sent_pst",
      tags$span(
        tags$b("Upload Help Desk Sent PST"), tags$br(),
        tags$small("Outgoing mail (.pst)")
      ),
      multiple = FALSE,
      accept = c(".pst")
    ),
    fileInput(
      "file_users_csv",
      tags$span(
        tags$b("Upload EDT users CSV"), tags$br(),
        tags$small("Name, Email columns")
      ),
      multiple = FALSE,
      accept = c(".csv")
    ),
    fileInput(
      "file_excluded_csv",
      tags$span(
        tags$b("Upload sender exclusion CSV"), tags$br(),
        tags$small("Name, Email columns (help desk staff / system to exclude)")
      ),
      multiple = FALSE,
      accept = c(".csv")
    ),
    fileInput(
      "file_overrides_csv",
      tags$span(
        tags$b("Upload Name to email translator CSV"), tags$br(),
        tags$small("sender_name, email columns")
      ),
      multiple = FALSE,
      accept = c(".csv")
    ),
...
server <- function(input, output, session) {
  ...
  # ---- Help Desk PST analysis (Section 2.1) ----
  # Runs the Outlook analyzer pipeline on the three uploaded files. Wrapped so a
  # missing Java runtime / unreadable PST surfaces as a clean message rather than
  # crashing the app. Returns list(linked, inbox_missing) or NULL.
  helpdesk_data <- reactive({
    # Soft guard: if files aren't all uploaded, return NULL instead of req()-aborting.
    if (is.null(input$file_inbox_pst) ||
      is.null(input$file_sent_pst) ||
      is.null(input$file_users_csv) ||
      is.null(input$file_excluded_csv) ||
      is.null(input$file_overrides_csv)) {
      return(NULL)
    }

    if (tools::file_ext(input$file_inbox_pst$name) != "pst" ||
      tools::file_ext(input$file_sent_pst$name) != "pst" ||
      tools::file_ext(input$file_users_csv$name) != "csv" ||
      tools::file_ext(input$file_excluded_csv$name) != "csv" ||
      tools::file_ext(input$file_overrides_csv$name) != "csv") {
      return(NULL)
    }

    # req(input$file_inbox_pst, input$file_sent_pst, input$file_users_csv)
    # req(tools::file_ext(input$file_inbox_pst$name) == "pst")
    # req(tools::file_ext(input$file_sent_pst$name) == "pst")
    # req(tools::file_ext(input$file_users_csv$name) == "csv")

    tryCatch(
      {
        inbox_pst <- hd_stage_pst(input$file_inbox_pst$datapath)
        sent_pst <- hd_stage_pst(input$file_sent_pst$datapath)
        hd_run_pipeline(
          inbox_pst, sent_pst,
          input$file_users_csv$datapath,
          input$file_excluded_csv$datapath,
          input$file_overrides_csv$datapath
        )
      },
      error = function(e) {
        warning("Help Desk PST analysis failed: ", e$message)
        NULL
      }
    )
  })
  ...
  output$plot_weekly_job_counts <- plotly::renderPlotly({
    ...
    fig <- fig %>%
      layout(
        barmode = "group",
        xaxis = list(title = list(text = "Date", standoff = 1), type = "date"),
  ...
  output$plot_sync_benchmarks <- plotly::renderPlotly({
    ...
    plotly::plot_ly() %>%
      ...
      plotly::layout(
        xaxis = list(
          title = list(text = "Day", standoff = 1),
          type = "date"
        ),
        yaxis = list(
  ...
  fmt_int <- function(x) ifelse(is.finite(x), formatC(as.integer(round(x)), big.mark = ",", format = "d"), "—")
  ...
  # ---- Report: HTML builder ----
  generate_report_html <- function(df, df_rej, date_range, helpdesk = NULL) {
    # --- Header metadata ---
    ...
    if (is.null(df) || nrow(df) == 0) {
      return(paste0(
        ...
        "<title>Periodic EnMoDS Data and Issues Summary</title></head>",
        '<body style="font-family:Calibri,Arial,sans-serif;max-width:8.5in;margin:1in auto;">',
        "<h1>Periodic EnMoDS Data and Issues Summary</h1>",
    ...
    # AQI stats
    mean_aqi <- if (nrow(df_t2) > 0) mean(df_t2$aqi_per_200, na.rm = TRUE) else NA_real_
    p10_aqi <- if (nrow(df_t2) > 0) quantile(df_t2$aqi_per_200, 0.10, na.rm = TRUE) else NA_real_
    p90_aqi <- if (nrow(df_t2) > 0) quantile(df_t2$aqi_per_200, 0.90, na.rm = TRUE) else NA_real_

    # Salus stats
    mean_salus <- if (nrow(df_t2) > 0) mean(df_t2$salus_per_200, na.rm = TRUE) else NA_real_
    p10_salus <- if (nrow(df_t2) > 0) quantile(df_t2$salus_per_200, 0.10, na.rm = TRUE) else NA_real_
    p90_salus <- if (nrow(df_t2) > 0) quantile(df_t2$salus_per_200, 0.90, na.rm = TRUE) else NA_real_
    ...
    # --- 2 Help Desk Metrics (computed from uploaded PST analysis) ---
    # Compute the same Vancouver-time window the rest of the report uses.
    if (!is.null(date_range)) {
      hd_start_van <- as.POSIXct(format(date_range[1], "%Y-%m-%d"),
        format = "%Y-%m-%d", tz = "America/Vancouver"
      )
      hd_end_van <- as.POSIXct(format(date_range[2], "%Y-%m-%d"),
        format = "%Y-%m-%d", tz = "America/Vancouver"
      ) + 86399
    } else {
      hd_start_van <- as.POSIXct("1900-01-01", tz = "America/Vancouver")
      hd_end_van <- as.POSIXct("2999-12-31", tz = "America/Vancouver")
    }

    if (!is.null(helpdesk) && !is.null(helpdesk$linked)) {
      hd_metrics <- hd_compute_metrics(
        helpdesk$linked, helpdesk$inbox_missing, hd_start_van, hd_end_van
      )
      requests_addressed_val <- fmt_int(hd_metrics$requests_addressed)

      # Statistical helpdesk response time: median (mean, p10, p90) in days.
      rt <- hd_metrics$response_time
      response_time_val <- if (is.null(rt) || is.na(rt$median) || rt$n == 0) {
        "—"
      } else {
      ...
      # oldest_pending_val <- if (length(hd_metrics$oldest_pending) == 0 ||
      #   all(is.na(hd_metrics$oldest_pending)) ||
      #   is.infinite(hd_metrics$oldest_pending)) {
      #   "—"
      # } else {
      #   format(hd_metrics$oldest_pending, "%Y-%m-%d %H:%M", tz = "America/Vancouver")
      # }

      oldest_pending_val <- if (length(hd_metrics$oldest_pending) == 0 ||
        all(is.na(hd_metrics$oldest_pending)) ||
        all(is.infinite(hd_metrics$oldest_pending))) {
        "—"
      } else {
        # oldest_pending is already a character vector of "%Y-%m-%d"
        # strings (formatted inside hd_compute_metrics), so just join it.
        paste(hd_metrics$oldest_pending, collapse = ", ")
      }
    } else {
      requests_addressed_val <- "—"
      response_time_val <- "—"
      oldest_pending_val <- "—"
    }

    helpdesk_rows <- paste0(
      "<tr>",
      "<td>Requests addressed in the selected period</td>",
      "<td class='num'>", requests_addressed_val, "</td>",
      "</tr>",
      "<tr>",
      "<td>Statistical helpdesk response time (in days)</td>",
      "<td class='num'>", response_time_val, "</td>",
      "</tr>",
      "<tr>",
      "<td>Timestamps of the five oldest incoming requests pending for response</td>",
      "<td>", oldest_pending_val, "</td>",
      "</tr>",
      # still have to add this manually
      "<tr>",
      "<td>Number of cumulative flagged requests pending for response</td>",
      "<td class='num'>100</td>",
      "</tr>"
    )
...
<head>
<meta charset='UTF-8'>
<title>Periodic EnMoDS Data and Issues Summary</title>
<style>
...
<h3>1.2 Performance indicator for upload speed to EDT (minutes per 200 observations)</h3>
<p class='note'>Median values for SUBMITTED files with results_count &ge; 25.</p>
<table>
  <tr><th>Metric</th><th>Value</th></tr>
   <tr>
    <td>Benchmark time set</td>
    <td class='num'>", "1 min", "</td>
  </tr>
  <tr>
  <td>Time taken by AQI</td><td>", paste0(
        fmt_min(median_aqi),
        " (Mean: ", fmt_min(mean_aqi),
        ", P10: ", fmt_min(p10_aqi),
        ", P90: ", fmt_min(p90_aqi)
      ), ")", "</td>
  </tr>
  <tr>
  <td>Time taken by Salus</td><td>", paste0(
        fmt_min(median_salus),
        " (Mean: ", fmt_min(mean_salus),
        ", P10: ", fmt_min(p10_salus),
        ", P90: ", fmt_min(p90_salus)
      ), ")", "</td>
  </tr>
</table>
...
<h2>2. EnMoDS Help Desk Summary</h2>

<h3>2.1 EnMoDS Help Desk Performance Metrics</h3>
<p class='note'>Median values for statistical response times &ge; 25.</p>
<table>
  <thead>
    <tr>
      <th>Email Type</th>
...
<table>
  ...
  output$download_report <- downloadHandler(
    filename = function() {
      paste0("Periodic_EnMoDS_Data_and_Issues_Summary_", Sys.Date(), ".html")
    },
    content = function(file) {
      message("Starting report download")

      tryCatch(
        {
          df <- tryCatch(base_filtered(), error = function(e) {
            message("base_filtered failed: ", e$message)
            NULL
          })
          df_rej <- tryCatch(rejected_followup(), error = function(e) {
            message("rejected_followup failed: ", e$message)
            NULL
          })
          # helpdesk <- NULL
          helpdesk <- tryCatch(helpdesk_data(), error = function(e) {
            message("helpdesk failed: ", e$message)
            NULL
          })
          html <- generate_report_html(df, df_rej, input$date_range, helpdesk)
